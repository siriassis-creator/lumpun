...
const HomeView = () => (
  <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-6 relative overflow-hidden">
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 max-w-4xl w-full relative z-10">
      ...
      <AppIcon icon={Bell} label="แจ้งเตือน Line" path="/notifications" color="bg-gradient-to-br from-pink-500 to-pink-600" />
    ...
    <PageTemplate title="📊 สรุปภาพรวมระบบ">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in duration-500">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex items-center justify-between">
          <div>
            <h4 className="text-slate-500 font-bold text-sm uppercase">สมาชิกทั้งหมด</h4>
            <p className="text-4xl font-black text-blue-600 mt-2">{memberCount} <span className="text-lg text-slate-400">คน</span></p>
          </div>
          <Users size={48} className="text-blue-100" />
        </div>
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex items-center justify-between">
          <div>
            <h4 className="text-slate-500 font-bold text-sm uppercase">กิจกรรมเดือนนี้</h4>
            <p className="text-4xl font-black text-orange-500 mt-2">{eventsThisMonth} <span className="text-lg text-slate-400">ครั้ง</span></p>
          </div>
          <Calendar size={48} className="text-orange-100" />
        </div>
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex items-center justify-between">
          <div>
            <h4 className="text-slate-500 font-bold text-sm uppercase">สถานะเชื่อมต่อ</h4>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 flex flex-col">
          <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-orange-50/50 rounded-t-3xl">
            <h3 className="text-lg font-bold text-orange-800 flex items-center gap-2">
              <Calendar size={20} className="text-orange-600" /> กิจกรรมเร็วๆ นี้
            </h3>
            <Link to="/calendar" className="text-sm font-bold text-orange-600 hover:text-orange-700">ดูปฏิทิน</Link>
          </div>
                  ...
                  return (
                    <div key={ev.id} className="flex gap-4 items-start p-4 rounded-2xl border border-slate-100 hover:bg-slate-50 transition">
                      <div className="bg-orange-100 text-orange-700 rounded-xl px-3 py-2 text-center min-w-[60px] shrink-0">
                        <p className="text-xs font-bold uppercase">{evtDate.toLocaleString('th-TH', { month: 'short' })}</p>
                        <p className="text-xl font-black">{evtDate.getDate()}</p>
                      </div>
                      <div className="flex-1">
                        <h4 className="font-bold text-slate-800">{ev.title}</h4>
                        <div className="flex items-center gap-4 mt-1 text-xs text-slate-500">
                          <span className="flex items-center gap-1"><Clock size={14} /> {ev.startTime} - {ev.endTime}</span>
                          {ev.location && <span className="flex items-center gap-1 truncate max-w-[120px]"><MapPin size={14} /> {ev.location}</span>}
                        </div>
                      </div>
                    </div>
                  )
...
const MembersView = () => {
  ...
  const initialForm = {
    imageUrl: '', firstName: '', lastName: '', nickname: '', age: '', position: '',
    affiliation: 'คณะผู้บริหารสภาเด็กและเยาวชนจังหวัดลำพูน', address: '', district: 'เมืองลำพูน',
    phone: '', memberId: '', height: '', weight: '', emergencyPhone: ''
  };
  ...
  const handleSaveMember = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && editingMemberId) {
        ...
        await addDoc(collection(db, 'members'), { ...formData, createdAt: serverTimestamp() });
      ...
      <div className="bg-white p-4 md:p-6 rounded-3xl shadow-sm border border-slate-200 flex-1 flex flex-col h-full animate-in fade-in duration-500">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 border-b border-slate-100 pb-5">
          ...
          <button 
            ...
            className={`${isFormVisible ? 'bg-slate-500 hover:bg-slate-600' : 'bg-indigo-600 hover:bg-indigo-700'} text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 transition shadow-sm w-full md:w-auto justify-center`}
          >
            {isFormVisible ? <><X size={18}/> ยกเลิก</> : <><UserPlus size={18} /> เพิ่มสมาชิกใหม่</>}
          </button>
        </div>
        ...
        {isFormVisible && (
          <div className="mb-8 p-6 bg-indigo-50/50 border border-indigo-100 rounded-3xl animate-in slide-in-from-top-4 fade-in duration-300">
            <div className="flex items-center gap-2 mb-6 border-b border-indigo-100 pb-3">
              ...
              <h3 className="text-lg font-bold text-indigo-900">{isEditing ? 'แก้ไขข้อมูลสมาชิก' : 'กรอกข้อมูลสมาชิกใหม่'}</h3>
            </div>
            <form onSubmit={handleSaveMember} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                <div className="md:col-span-2 xl:col-span-4">
                  <label className="block text-xs font-bold text-slate-700 mb-1">ลิงก์ URL รูปถ่าย (เว็บฝากรูป)</label>
                  <input required type="url" placeholder="https://..." value={formData.imageUrl} onChange={e => setFormData({...formData, imageUrl: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">เลขประจำตัวสมาชิก *</label>
                  <input required type="text" value={formData.memberId} onChange={e => setFormData({...formData, memberId: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm" placeholder="เช่น LPN-001" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 mb-1">ตำแหน่ง *</label>
                  <input required type="text" value={formData.position} onChange={e => setFormData({...formData, position: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm" placeholder="เช่น ประธาน, รองประธาน, สมาชิก" />
                </div>
                <div className="md:col-span-2 xl:col-span-4">
                  <label className="block text-xs font-bold text-slate-700 mb-1">สังกัด *</label>
                  <select value={formData.affiliation} onChange={e => setFormData({...formData, affiliation: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm font-bold text-indigo-800 bg-white">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-4">
                <div className="col-span-2">
                  ...
                  <input required type="text" value={formData.firstName} onChange={e => setFormData({...formData, firstName: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm" placeholder="ชื่อ" />
                </div>
                <div className="col-span-2">
                  <label className="block text-xs font-bold text-slate-700 mb-1">นามสกุล *</label>
                  <input required type="text" value={formData.lastName} onChange={e => setFormData({...formData, lastName: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm" placeholder="นามสกุล" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">ชื่อเล่น</label>
                  <input type="text" value={formData.nickname} onChange={e => setFormData({...formData, nickname: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm" placeholder="ชื่อเล่น" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">อายุ (ปี)</label>
                  <input type="number" value={formData.age} onChange={e => setFormData({...formData, age: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm" placeholder="ตัวเลข" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">ส่วนสูง (ซม.)</label>
                  <input type="number" value={formData.height} onChange={e => setFormData({...formData, height: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm" placeholder="ตัวเลข" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">น้ำหนัก (กก.)</label>
                  <input type="number" value={formData.weight} onChange={e => setFormData({...formData, weight: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm" placeholder="ตัวเลข" />
                </div>
              </div>
              <div className="h-px bg-slate-200"></div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">อำเภอที่อยู่ *</label>
                  <select value={formData.district} onChange={e => setFormData({...formData, district: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm font-bold text-slate-700 bg-white">
                    {districts.map(d => <option key={d} value={d}>อ.{d}</option>)}
                  </select>
                </div>
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 mb-1">ที่อยู่แบบเต็ม</label>
                  <input type="text" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm" placeholder="บ้านเลขที่ หมู่ หมู่บ้าน ตำบล" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">เบอร์โทรศัพท์ติดต่อ *</label>
                  <input required type="tel" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-indigo-400 text-sm" placeholder="08X-XXX-XXXX" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-red-600 mb-1">เบอร์ติดต่อฉุกเฉิน *</label>
                  <input required type="tel" value={formData.emergencyPhone} onChange={e => setFormData({...formData, emergencyPhone: e.target.value})} className="w-full border border-red-200 bg-red-50 p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-red-400 text-sm" placeholder="เบอร์ผู้ปกครอง/ญาติ" />
                </div>
              </div>
              <div className="flex justify-end gap-3 pt-5 border-t border-indigo-100">
                <button type="button" onClick={handleCloseForm} className="px-6 py-2.5 text-slate-600 font-bold hover:bg-slate-200 rounded-xl transition">
                  ยกเลิก
                </button>
                <button type="submit" disabled={isLoading} className="px-8 py-2.5 bg-indigo-600 text-white font-bold rounded-xl shadow-md hover:bg-indigo-700 transition flex items-center gap-2">
                  <Save size={18}/>
                  {isLoading ? 'กำลังประมวลผล...' : (isEditing ? 'แก้ไขข้อมูล' : 'บันทึกข้อมูล')}
                </button>
              </div>
            </form>
          </div>
              ...
              {filteredMembers.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-20 text-center text-slate-400">
                    ...
                    <p className="text-sm mt-1">กดปุ่ม "เพิ่มสมาชิกใหม่" หรือกรองการค้นหาใหม่ดูนะครับ</p>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={m.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="p-4 text-center">
                      <div className="w-14 h-14 rounded-full overflow-hidden bg-slate-200 mx-auto border-4 border-white shadow-md flex items-center justify-center">
                        {m.imageUrl ? (
                          <img src={m.imageUrl} alt={m.firstName} className="w-full h-full object-cover" onError={(e: any) => e.target.src = 'https://via.placeholder.com/150'} />
                        ) : (
                    ...
                    <td className="p-4">
                      ...
                      <p className="text-xs text-slate-500 font-medium mt-1">
                        ชื่อเล่น: <span className="text-indigo-600 font-bold">{m.nickname || '-'}</span> | อายุ: {m.age || '-'} ปี
                      </p>
                    </td>
                    <td className="p-4">
                      <span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-xs font-mono font-bold border border-slate-200">{m.memberId || 'N/A'}</span>
                      <p className="font-bold text-indigo-700 mt-2 text-base">{m.position}</p>
                    </td>
                    <td className="p-4">
                      <p className="text-xs font-bold text-slate-700 truncate max-w-[200px] md:max-w-xs">{m.affiliation}</p>
                      <p className="text-xs text-orange-700 font-bold mt-1.5 bg-orange-100 inline-block px-2.5 py-1 rounded-full">อ.{m.district}</p>
                    </td>
                    <td className="p-4">
                      <p className="font-mono text-slate-700 font-bold text-base">{m.phone}</p>
                      <p className="text-xs text-red-600 mt-1 bg-red-50 inline-block px-2 py-0.5 rounded">ฉุกเฉิน: {m.emergencyPhone}</p>
                    </td>
                    <td className="p-4 text-center">
                      <div className="flex gap-2 justify-center">
                        <button onClick={() => handleStartEdit(m)} className="p-2.5 text-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-colors" title="แก้ไขข้อมูล"><Edit2 size={18} /></button>
                        <button onClick={() => handleDelete(m.id, m.firstName)} className="p-2.5 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors" title="ลบข้อมูล"><Trash2 size={18} /></button>
                      </div>
                    </td>
                  </tr>
                ))
...
const CalendarView = () => {
  ...
  const initialForm = {
    title: '', date: '', startTime: '09:00', endTime: '12:00', location: '', description: ''
  };
  const [formData, setFormData] = useState(initialForm);
      ...
      <div className="bg-white p-4 md:p-6 rounded-3xl shadow-sm border border-slate-200 flex-1 flex flex-col h-full">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 border-b border-slate-100 pb-5">
          <div className="flex items-center gap-4">
            <button onClick={handleToday} className="px-4 py-2 border border-slate-300 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition">
              วันนี้
            </button>
            <div className="flex items-center gap-1 bg-slate-100 rounded-xl p-1">
              <button onClick={handlePrevMonth} className="p-2 hover:bg-white rounded-lg text-slate-600 transition"><ChevronLeft size={20}/></button>
              <button onClick={handleNextMonth} className="p-2 hover:bg-white rounded-lg text-slate-600 transition"><ChevronRight size={20}/></button>
            </div>
        ...
        <div className="flex-1 flex flex-col bg-slate-200 border border-slate-200 rounded-2xl overflow-hidden">
          <div className="grid grid-cols-7 gap-px bg-slate-200">
            {dayNames.map(d => (
              <div key={d} className="bg-slate-50 py-3 text-center text-xs font-bold text-slate-500 uppercase">{d}</div>
            ))}
          </div>
            ...
            {calendarCells.map((day, idx) => {
              ...
              return (
                <div 
                  ...
                  className={`bg-white min-h-[100px] p-1.5 transition ${day ? 'hover:bg-orange-50/30 cursor-pointer' : 'opacity-50 cursor-default'}`}
                >
                  {day && (
                    <>
                      <div className={`text-right text-sm font-bold mb-1 ${isToday ? 'text-white' : 'text-slate-600'}`}>
                        <span className={`${isToday ? 'bg-orange-500 rounded-full w-7 h-7 flex items-center justify-center ml-auto shadow-md' : 'inline-block w-7 text-center'}`}>{day}</span>
                      </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col animate-in slide-in-from-bottom-4">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-orange-50">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-bold text-lg text-orange-900">{editingId ? 'แก้ไขกิจกรรม' : 'เพิ่มกิจกรรมใหม่'}</h3>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-orange-200 rounded-full text-slate-500 transition-colors"><X size={20} /></button>
            </div>
            
            <form onSubmit={handleSaveEvent} className="p-6 space-y-4">
              <div>
                <input required type="text" placeholder="เพิ่มชื่อกิจกรรม *" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full border-b-2 border-slate-200 px-2 py-3 outline-none focus:border-orange-500 text-xl font-bold text-slate-800 placeholder-slate-400" />
              </div>
              
              <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <Clock className="text-slate-400" size={20} />
                <input required type="date" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="border p-2 rounded-xl text-sm font-bold text-slate-700 outline-none" />
                <div className="flex items-center gap-2">
                  <input required type="time" value={formData.startTime} onChange={e => setFormData({...formData, startTime: e.target.value})} className="border p-2 rounded-xl text-sm font-bold text-slate-700 outline-none" />
                  <span className="text-slate-400">-</span>
                  <input required type="time" value={formData.endTime} onChange={e => setFormData({...formData, endTime: e.target.value})} className="border p-2 rounded-xl text-sm font-bold text-slate-700 outline-none" />
                </div>
              </div>

              <div className="flex items-center gap-4 px-2">
                <MapPin className="text-slate-400" size={20} />
                <input type="text" placeholder="เพิ่มสถานที่" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} className="w-full border-b border-slate-200 py-2 outline-none focus:border-orange-500 text-sm" />
              </div>

              <div className="flex items-start gap-4 px-2">
                <FileText className="text-slate-400 mt-2" size={20} />
                <textarea rows={3} placeholder="เพิ่มคำอธิบายรายละเอียด..." value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full border-b border-slate-200 py-2 outline-none focus:border-orange-500 text-sm resize-none" />
              </div>

              <div className="flex justify-between items-center pt-6 mt-4">
                {editingId ? (
                  <button type="button" onClick={handleDeleteEvent} className="text-red-500 hover:bg-red-50 px-4 py-2 rounded-xl font-bold transition flex items-center gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="submit" disabled={isLoading} className="px-6 py-2.5 bg-orange-500 text-white font-bold rounded-xl shadow-md hover:bg-orange-600 transition flex items-center gap-2">
...
const AttendanceView = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'attendance', selectedEventId), (docSnap) => {
      if (docSnap.exists()) {
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-full max-h-[calc(100vh-100px)]">
        <div className="w-full lg:w-1/3 bg-white p-4 md:p-6 rounded-3xl shadow-sm border border-slate-200 flex flex-col h-full">
          <h3 className="font-bold text-lg text-slate-800 mb-4 flex items-center gap-2">
            <Calendar className="text-emerald-600" size={20}/> เลือกกิจกรรมที่ต้องการ
          </h3>
          <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3">
            {events.length === 0 ? (
              <p className="text-sm text-slate-400 text-center mt-10">ยังไม่มีกิจกรรมในระบบ</p>
            ) : (
              events.map(ev => {
                ...
                return (
                  <div 
                    ...
                    className={`p-4 rounded-2xl border cursor-pointer transition-all ${
                      isSelected ? 'bg-emerald-50 border-emerald-500 shadow-md ring-1 ring-emerald-500' : 'bg-white border-slate-200 hover:bg-slate-50'
                    }`}
                  >
                    <p className="text-xs font-bold text-emerald-600 mb-1">{ev.date} | {ev.startTime}</p>
                    <h4 className={`font-bold ${isSelected ? 'text-emerald-900' : 'text-slate-700'}`}>{ev.title}</h4>
                  </div>
                )
              })
            )}
              ...
              <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1 flex items-center gap-2">
                    <Calendar size={14}/> {selectedEvent.date} ({selectedEvent.startTime} - {selectedEvent.endTime})
                  </p>
                </div>
                <button 
                ...
                >
                  {isSaving ? 'กำลังบันทึก...' : <><Save size={18} /> บันทึกการเข้าร่วม</>}
                </button>
              </div>
                ...
                <table className="w-full text-left text-sm whitespace-nowrap min-w-max">
                  <thead className="bg-slate-100 text-slate-600 text-[11px] uppercase font-bold sticky top-0 z-10 shadow-sm">
                    <tr>
                      ...
                      <th className="p-4 text-center border-l border-slate-200">มา</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {members.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="p-10 text-center text-slate-400">ไม่พบรายชื่อสมาชิกในระบบ</td>
                      </tr>
                    ) : (
                      members.map((m, idx) => {
                        const status = attendanceData[m.id] || 'none';
                        return (
                          <tr key={m.id} className={`transition-colors ${status !== 'none' ? 'bg-emerald-50/20' : 'hover:bg-slate-50'}`}>
                            <td className="p-4 pl-6 font-mono text-slate-500">{idx + 1}</td>
                            <td className="p-4 font-bold text-slate-800">{m.firstName} {m.lastName} <span className="text-xs text-slate-400 font-normal">({m.nickname})</span></td>
                            <td className="p-4 text-slate-600 text-xs">{m.position}</td>
                            <td className="p-4 text-center border-l border-slate-100">
                              <input type="radio" name={`status-${m.id}`} checked={status === 'present'} onChange={() => handleStatusChange(m.id, 'present')} className="w-5 h-5 accent-emerald-600 cursor-pointer" />
                            </td>
                            <td className="p-4 text-center">
                              <input type="radio" name={`status-${m.id}`} checked={status === 'leave'} onChange={() => handleStatusChange(m.id, 'leave')} className="w-5 h-5 accent-orange-500 cursor-pointer" />
                            </td>
                            <td className="p-4 text-center">
                              <input type="radio" name={`status-${m.id}`} checked={status === 'late'} onChange={() => handleStatusChange(m.id, 'late')} className="w-5 h-5 accent-yellow-500 cursor-pointer" />
                            </td>
                            <td className="p-4 text-center pr-6">
                              <input type="radio" name={`status-${m.id}`} checked={status === 'absent'} onChange={() => handleStatusChange(m.id, 'absent')} className="w-5 h-5 accent-red-600 cursor-pointer" />
                            </td>
                          </tr>
                        )
                      })
